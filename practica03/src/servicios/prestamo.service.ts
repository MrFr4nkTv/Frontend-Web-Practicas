...
constructor(private readonly repositorio: PrestamoRepository) {}
  ...
  async crear(dto: CrearPrestamoDto): Promise<Prestamo> {
    ...
    const delLibro = await this.repositorio.findByLibro(dto.libroId);
    
    // 2. juntar los ejemplares que ya estan fuera
    const pfuera = delLibro
      .filter(p => p.estado === 'activo' || p.estado === 'vencido')
      .flatMap(p => p.ejemplares);

      const choque = dto.ejemplares.find(e => pfuera.includes(e));
      if (choque !== undefined) {
      ...
      const prestamo: Prestamo = { 
        ...
        socioId: dto.socioId,
        estado: 'activo',
        costoReposicion: 350, // valor por defecto
      };
